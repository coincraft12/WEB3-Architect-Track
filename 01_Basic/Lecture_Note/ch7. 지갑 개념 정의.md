# 7강. 지갑 개념 정의: “지갑은 돈통이 아니라 키·정책·행동 인터페이스다”

## 2) 도입 멘트 (강의 시작 2~3분 대본)
> 많은 분들이 지갑을 ‘돈이 들어있는 곳’이라고 생각합니다.  
> 그런데 블록체인에서 돈은 지갑에 들어있지 않아요.  
> 돈은 체인 위 상태(state)에 있고,  
> 지갑은 그 상태를 바꾸기 위한 **열쇠(키)** 를 들고 있을 뿐입니다.  
>  
> 그래서 지갑은 사실 돈통이 아니라,  
> **키를 관리하고, 서명하고, 사용자가 실수하지 않게 돕는 UI**입니다.  
> 오늘은 지갑을 ‘정확한 구조’로 정의해봅시다.

## 0) 이 강의의 한 문장
**지갑(Wallet)은 자산을 ‘보관’하는 상자가 아니라, 키를 안전하게 관리하고 ‘서명된 행동’을 만들며, 사용자의 정책(권한/한도/보안)을 적용하는 인터페이스다.**  
돈은 지갑 안에 있는 게 아니라 **체인(장부)의 상태(State)** 로 존재한다.

---

## 1) 강의 목표 (수강생이 끝나고 할 수 있어야 하는 것)
이 강의를 마치면 수강생은:

1. “지갑=자산 보관함”이라는 오해를 깨고, 지갑을 **키/정책/인터페이스**로 정의할 수 있다.  
2. 지갑과 계정의 차이를 구조적으로 설명한다(중앙화 계정 vs 키 기반).  
3. 지갑의 핵심 기능 6가지(키관리, 서명, 네트워크 선택, 수수료, 승인/권한, 복구)를 역할별로 이해한다.  
4. 지갑 유형(수탁형/비수탁형, 소프트웨어/하드웨어)을 위험 모델(권한-책임, 단일 실패점)로 비교할 수 있다.  
5. 다음 강의(주소)로 연결: “주소는 지갑이 아니라 키의 표현이며, 지갑 하나가 여러 주소를 다룰 수 있다”를 이해한다.

---



---

## 3) 핵심 정의: 지갑은 3가지의 결합체
지갑 = (Key Manager) + (Policy Engine) + (Action Interface)

### A. Key Manager(키 관리자)
- 개인키/시드의 생성·보관·보호
- 서명 기능의 접근 제어(잠금, 생체인증, PIN 등)

### B. Policy Engine(정책 엔진)
- 어떤 행동을 허용할지, 어떤 위험을 막을지  
- 예: 승인(approve) 경고, 스캠 탐지, 주소 라벨링, 한도, 멀티시그 정책 등

### C. Action Interface(행동 인터페이스)
- 사용자가 “무엇을 할지” 선택하고  
- 트랜잭션(명령)을 구성하고  
- 서명 후 전송하는 UI/UX

**암기 문장**
- 지갑은 “자산 저장소”가 아니라 “행동 생성기”다.

---

## 4) 가장 중요한 오해 깨기: “지갑에 코인이 들어있다”는 표현의 진짜 의미
사실 관계를 정확히 쓰면:

- 자산은 체인의 상태(state)에 존재한다  
- 주소는 상태를 가리키는 식별자다  
- 지갑은 그 주소의 상태를 바꾸는 서명 권한(키)을 통제한다

그래서 “지갑에 코인이 있다”는 말은 편의상:
- “내 키가 통제하는 주소의 상태에 코인이 있다”를 줄인 표현이다.

---

## 5) 지갑이 하는 일 6가지(구조 설계자 관점)
### 1) 키 생성/보관/보호(보안의 심장)
- 시드 생성, 백업, 암호화 저장, 잠금
- 단일 실패점(SPOF)을 어떻게 줄이는지(하드웨어/멀티시그/분산복구)

### 2) 트랜잭션 구성(행동 만들기)
- To/Value/Data/Nonce/Gas/ChainId 구성  
- “무엇을 승인하는지”를 사람이 이해할 수 있게 번역

### 3) 서명(Sign)
- 개인키로 승인
- “내가 동의했다”의 기술적 증거 생성

### 4) 네트워크/체인 선택(세계 선택)
- 같은 주소처럼 보여도 체인이 다르면 전혀 다른 세계  
- 지갑은 “어느 세계에서 행동할지”를 고르게 한다

### 5) 수수료/가스 전략(실행 조건 설정)
- 가스는 비용이면서 실행 조건  
- 지갑은 ‘실행 가능성’과 ‘비용’을 절충하게 만든다

### 6) 승인/권한 관리(가장 위험한 UX 지점)
- approve / permit / allowance 등 권한 위임을 관리
- 많은 사고가 “송금”이 아니라 “승인 과부여”에서 터진다

**설계자 포인트**
- 지갑 UX의 품질은 “보안”과 직결된다.  
  사용자가 실수하기 쉬운 구조를 얼마나 줄여주느냐가 지갑의 실력이다.

---

## 6) 지갑 vs 계정: 구조 비교(중앙화/WEB3)
### 6.1 중앙화 계정(은행/거래소 계정)
- 로그인 = 접근 권한 획득
- 복구/정지/환불/분쟁조정 내장
- 대신 기관이 권한을 가진다(동결/차단/규칙변경 가능)

### 6.2 WEB3 지갑(비수탁형)
- 서명 = 소유권 행사(행동 승인)
- 복구/구제는 제한적(사용자 책임)
- 대신 사용자가 통제한다(기관이 임의로 동결하기 어렵다)

**암기 문장**
- 계정은 “기관이 주는 권한”  
- 지갑은 “내 키로 행사하는 소유권”

---

## 7) 지갑 유형을 “기능”이 아니라 “위험 모델”로 분류하기
초급에서 중요한 건 브랜드/제품이 아니라 구조다.

### A. 수탁형(Custodial) 지갑
- 키를 서비스가 보관(사용자는 접근 권한만 가짐)
- 장점: 복구/고객지원/단순 UX
- 단점: 동결/차단/규칙변경/해킹(중앙화 리스크)

**구조 요약**
- 편리함 ↑ / 통제권 ↓ / 중앙화 리스크 ↑

### B. 비수탁형(Non-custodial) 지갑
- 키를 사용자가 통제
- 장점: 자기 통제, 검열 저항성(상대적으로)
- 단점: 분실/피싱/승인 실수의 책임이 사용자에게

**구조 요약**
- 통제권 ↑ / 복구 난이도 ↑ / 사용자 책임 ↑

### C. 하드웨어 vs 소프트웨어(키 격리 관점)
- 하드웨어 지갑: 키를 별도 장치에 격리(유출 위험 감소)  
- 소프트웨어 지갑: 편리하지만 단말기 침해에 취약할 수 있음

**핵심**
- 하드웨어는 “키가 인터넷과 멀어지는” 설계다.

---

## 8) 지갑을 “보안 정책 장치”로 보기(초급에 필요한 최소 보안 프레임)
지갑을 쓸 때 가장 중요한 질문 3개:

1) **키는 어디에 있는가?** (내 폰? 서비스 서버? 하드웨어?)  
2) **서명은 어떤 절차로 승인되는가?** (생체/PIN/2FA/멀티시그?)  
3) **승인(allowance/권한 위임)은 어떻게 관리되는가?** (경고/한도/만료/리뷰?)

이 3개를 잡으면 “지갑 이해”의 절반은 끝난다.

---

## 9) 미니 케이스(강의 중 10~15분)
### 케이스: “지갑이 털렸다”는 말의 실체
사실 “지갑이 털렸다”는 말은 보통 아래 중 하나다.

- 키/시드가 유출되었다(키 관리자 실패)  
- 사용자가 스캠 서명 요청을 승인했다(UI/정책 실패)  
- 과거 승인(approve)이 남아있었고 권한이 사용되었다(권한 관리 실패)  
- 네트워크/주소를 착각해 잘못 보냈다(인터페이스 실패)

**강의 포인트**
- 사고는 ‘코인’이 아니라 ‘서명/권한/정책’에서 터진다.  
- 지갑은 이 사고를 줄이기 위해 존재한다.

---

## 10) 미니 실습(20분) — 내 지갑을 “기능 지도”로 해부하기
### 실습 주제: 내가 쓰는 지갑 1개를 6기능 기준으로 점검
아래 질문에 답한다.

1) 키 백업(시드/복구)은 어떻게 되어 있나?  
2) 트랜잭션을 구성할 때 무엇을 보여주나? (To/Value/Data)  
3) 서명 승인 UX는 안전한가? (잠금/추가 확인)  
4) 네트워크 선택 실수를 막는 장치가 있나?  
5) 가스 설정을 어떻게 안내하나? (자동/수동/경고)  
6) 승인/권한(allowance) 관리는 어디서 확인하나?

**출력(산출물)**: “내 지갑 기능 점검표 1장 + 개선 아이디어 3개”

---

## 11) 과제
### 과제 1: 지갑 유형 비교 리포트(1페이지)
- 수탁형 vs 비수탁형을  
  - 통제권
  - 복구 가능성
  - 동결/차단 가능성
  - 사용자 실수 리스크
  - 단일 실패점
  관점으로 표로 비교하고 결론 5문장 작성

### 과제 2(선택): 지갑 UX 개선 제안 5개
- “사용자 실수를 줄이는 UX” 관점으로
- 승인(approve) 경고 강화, 주소 라벨링, 네트워크 착각 방지, 위험 서명 해석 등

---

## 12) 강의 마무리 30초 대본
> 오늘의 핵심은 이겁니다.  
> 지갑은 돈통이 아니라, 키·정책·행동 인터페이스다.  
> 돈은 체인의 상태에 있고, 지갑은 그 상태를 바꾸는 서명 권한을 관리합니다.  
>  
> 다음 강의에서는 지갑과 자주 혼동되는 개념—  
> **주소(Address)** 를 구조적으로 정의해보겠습니다.  
> “주소는 무엇이고, 왜 그렇게 생겼고, 무엇을 연결하는가”를요.

---

## 13) 퀴즈(12문항, 복습용)
1) 지갑을 3가지 구성요소(Key Manager/Policy Engine/Action Interface)로 정의하라.  
2) “코인은 지갑에 있다”는 표현을 정확한 문장으로 풀어 써라.  
3) 지갑이 하는 일 6가지를 쓰시오.  
4) 수탁형 지갑의 장점 2개, 단점 2개를 쓰시오.  
5) 비수탁형 지갑의 장점 2개, 단점 2개를 쓰시오.  
6) 하드웨어 지갑이 줄이는 리스크는 무엇인가?  
7) 지갑과 계정의 본질적 차이는 무엇인가?  
8) 사고가 ‘송금’보다 ‘승인’에서 많이 터지는 이유를 한 문장으로 써라.  
9) 지갑 UX가 보안과 직결되는 이유는 무엇인가?  
10) 네트워크 선택 실수가 치명적인 이유는 무엇인가?  
11) “키는 어디에 있는가?”가 중요한 이유는 무엇인가?  
12) 지갑 이해를 위한 핵심 질문 3가지를 쓰시오.

---

## 14) (집필 확장용) 이 강의를 책으로 옮기면 이렇게 된다
- 1. 지갑은 돈통이 아니다  
- 2. 키·정책·인터페이스: 지갑의 3요소  
- 3. 지갑의 6기능(키/구성/서명/네트워크/가스/권한)  
- 4. 수탁형 vs 비수탁형: 위험 모델로 비교하기  
- 5. 하드웨어 지갑의 구조적 의미(키 격리)  
- 6. 사고의 대부분은 서명/권한/UX에서 발생한다  
- 7. 실습: 내 지갑 기능 점검표 만들기  
- 8. 다음 장 예고: 주소는 무엇이고 무엇을 연결하는가
