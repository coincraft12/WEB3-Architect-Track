...
- 트랜잭션: 서명된 상태 변경 요청.
- Nonce: 동일 계정 트랜잭션 순서를 보장하는 일련번호.
- Data: 컨트랙트 호출 함수/파라미터를 담는 입력.
- Receipt: 실행 결과(status, gasUsed, logs)를 담은 결과 레코드.
- Logs(Event): 컨트랙트가 외부에 남기는 구조화된 실행 흔적.

## 헷갈리는 개념 보충
- 트랜잭션은 단순 기록이 아니라 "행동 요청(명령)"이다.
- 실행이 끝난 뒤에야 결과 기록(영수증/로그)이 남는다.
