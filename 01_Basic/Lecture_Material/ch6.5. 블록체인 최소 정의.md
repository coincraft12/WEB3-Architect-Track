...
- 상태(State): 체인의 현재 사실 집합(잔액/소유/컨트랙트 변수).
- 상태 전이(State Transition): 트랜잭션 처리 전후 상태 변화.
- 블록(Block): 일정 구간 트랜잭션과 메타데이터를 묶은 단위.
- 체인(Chain): 블록이 순서대로 연결된 기록 구조.
- 공유 상태 머신: 다수 노드가 동일 규칙으로 같은 상태를 재현하는 시스템.
